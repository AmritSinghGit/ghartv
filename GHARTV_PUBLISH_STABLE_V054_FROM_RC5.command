...
REPO="AmritSinghGit/ghartv"; REVIEW_TAG="v0.5.4-rc5"; STABLE_TAG="v0.5.4"
...
s=gradle.read_text(); s=re.sub(r'versionCode\s*=\s*\d+','versionCode = 15',s,count=1); s=re.sub(r'versionName\s*=\s*"[^"]+"','versionName = "0.5.4"',s,count=1); gradle.write_text(s)
state=root/'.ghartv-owner-state.json'; d=json.loads(state.read_text()); d.update({'current_candidate':'0.5.4','version_code':15,'candidate_release':'v0.5.4','owner_decision':'OWNER_APPROVED_FOR_STABLE_PUBLICATION','stable_update':'v0.5.4 / versionCode 15'}); state.write_text(json.dumps(d,indent=2)+'\n')
manifest=root/'RELEASE_MANIFEST.json'; m=json.loads(manifest.read_text()); m.update({'release':'v0.5.4','versionName':'0.5.4','versionCode':15,'ownerState':'PUBLISHED','candidate_sha':'STABLE_SOURCE_SHA_SET_BY_GIT_COMMIT','owner_decision':'PUBLISHED'}); manifest.write_text(json.dumps(m,indent=2)+'\n')
PY
...
import json,sys
p=Path(sys.argv[1]); d=json.loads(p.read_text()); d.update({'versionCode':15,'versionName':'0.5.4','apkUrl':'https://github.com/AmritSinghGit/ghartv/releases/latest/download/GharTV-Jio-Live.apk','sha256':sys.argv[2],'notes':'Family photo birthday splash, automatic bounded muted channel preview, corrected Simrat mapping, voice/programme search, scoped channel surfing and capability-gated transport controls.','publishedAt':datetime.now(timezone.utc).isoformat()}); p.write_text(json.dumps(d,indent=2)+'\n')
PY
...
- Stable update: v0.5.4 / versionCode 15
- Owner decision: PUBLISHED
EOF
command -v amrit-context >/dev/null 2>&1 && amrit-context handoff --file "$NOTE" >/dev/null 2>&1 || true
echo "GharTV v0.5.4 is published from the reviewed RC5 candidate. On the TV choose Jio account → Check for GharTV update."
