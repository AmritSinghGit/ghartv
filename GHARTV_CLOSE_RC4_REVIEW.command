...
NOTE="$HOME/Documents/Amrit Executive Memory/90 System/Operon Portfolio/Handoffs/Terminal Runs/ghartv/ghartv-v054-rc4-close-$STAMP.md"
mkdir -p "$(dirname "$NOTE")" "$HOME/.local/state/operon-terminal-runs/ghartv"
cat > "$NOTE" <<EOF
# GharTV v0.5.4 RC4 review closed
...
EOF
cp "$NOTE" "$HOME/.local/state/operon-terminal-runs/ghartv/ghartv-v054-rc4-close-$STAMP.md"
