...
TAG='v0.6.0-rc9'; CODE=26; VERSION='0.6.0-rc9-tv-first'; ASSET='GharTV-review-current.apk'
...
def identity(expected):
    ...
    if r.get('status')!='REVIEW_READY' or r.get('emulator')!='RC9_INSTALLED_BYTES_VERIFIED_AND_FOREGROUND':raise Hold('GREEN_INSTALLATION_NOT_READY')
...
def verify_green(expected,open_tv=False):
    ...
    if not all(x in badging for x in ("name='"+PACKAGE+"'","versionCode='26'","versionName='"+VERSION+"'")):raise Hold('APK_PACKAGE_VERSION_MISMATCH')
    if open_tv:
...
def validate_approval(body,r):
    ...
    if body.get('confirmation')!='PUBLISH EXACT CODE 26':raise Hold('EXPLICIT_PRODUCTION_APPROVAL_REQUIRED')
...
def publish(expected,body):
    ...
    feed={'versionCode':CODE,'versionName':VERSION,'apkUrl':f'https://github.com/{REPO}/releases/download/{TAG}/{ASSET}',
          ...
          'notes':'Owner-reviewed exact code26: network diagnostics, manual reports, Punjabi selection and release desk. Update in place; do not uninstall.',
    ...
    changed=github('repos/'+REPO+'/contents/update/latest.json','PUT',{'message':'release: promote owner-approved exact code26 APK; no rebuild','branch':'main','sha':blob,'content':encoded})
...
def finish_publication(source,h,commit,feed,already=False):
    ...
    try:
        ...
        if vault.is_dir():write_json(vault/'90 System/Operon Portfolio/Handoffs/Terminal Runs/ghartv'/('promotion-code26-'+source[:12]+'.json'),state)
    ...
    try:
        ...
        comment='Owner approved and published exact GharTV code26. Source `'+source+'`, signed APK SHA256 `'+h+'`. No rebuild. Public update feed readback verified. Physical TV install and hosted browser deployment are not established by this publication.'
