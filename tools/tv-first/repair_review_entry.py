...
if 'GHARTV_TAB_STAGE_INVENTORY_COMPLETE' not in s:
    s=s.replace("  var chosen=running.indexOf('Safari')", "  console.log('GHARTV_TAB_STAGE_INVENTORY_COMPLETE');\n  var chosen=running.indexOf('Safari')")
if 'GHARTV_TAB_STAGE_CREATE_' not in s:
    s=s.replace("    if(!app.running()){app.launch();delay(.5);}","    console.log('GHARTV_TAB_STAGE_CREATE_'+target.id);\n    if(!app.running()){app.launch();delay(1);}")
if 'GHARTV_TAB_STAGE_DONE' not in s:
    s=s.replace("  return JSON.stringify({status:'BROWSER_TABS_RECONCILED',results:result});", "  console.log('GHARTV_TAB_STAGE_DONE');\n  return JSON.stringify({status:'BROWSER_TABS_RECONCILED',results:result});")
s=s.replace('console.error(', 'console.log(')
