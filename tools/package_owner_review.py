...
for root in ('docs','web-player','telemetry/worker'):
 for p in Path(root).rglob('*'):
  ...
  files.append(p)
files.extend(Path(x) for x in ('GHARTV_LANE_PROGRESS.md','CURRENT_HANDOFF.md','REVIEW_060_RC6.md','tools/GharTVApkVerifier.java'))
with zipfile.ZipFile(bundle,'w',compression=zipfile.ZIP_DEFLATED) as z:
 for p in sorted(files):z.write(p,str(p))
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
m={'schema':'ghartv.review-manifest.v2','source_sha':source,'branch':'codex/ghartv-remove-auto-preview','pr':1,'version_name':'0.6.0-rc6-family-focus','version_code':22,'unsigned_sha256':sha(apk),'companion_sha256':sha(bundle),'production_unchanged':True,'owner_signed_apk_sha256':None,'owner_mac_run':'NOT_EXECUTED','ai_super_resolution':'NOT_IMPLEMENTED'}
...
readme="# GharTV RC6 — actual review package\n\nVersion0.6.0-rc6-family-focus / code22. Public TV remains RC8/code17.\nRun RUN_GHARTV_REVIEW.command after verifying the supplied checksum. This seeds the exact local artifact cache and updates the same managed launcher. No internet fetch is needed for the bundled preparation, signing or APK installation. Live channels and the collector still require connectivity. Existing signing.env and release key are reused locally; no new key/password prompt.\n\nThe development checkout stays read-only. The named existing AVD is reused. Existing settings and provider login are not cleared. The installed APK and its signature must match this candidate and the accepted public signer. It opens the same local owner/player service on8790. The local owner reader automatically finds the collector credential; its safe status card shows where the configuration belongs. Never paste credentials into a chat or public issue.\n\nBundled mode does not upload a signed APK, deploy the backend or promote an update. Existing code17 viewers will not receive code22 until owner review and explicit promotion. The screenshot/checks supplied in this package are not proof of your physical-TV playback.\n\nReview first-open Next-channel focus; Jio account → Appearance → Manage family; browser top-bar search and keyboard/fullscreen; owner Overview / Televisions / Playback health / Review tabs. Category surfing remains in the selected list. No automatic D1 reporting polls. EPG absence is labelled, not invented. Automatic video preview remains off. The look is static navy/cyan with clearer focus and readable text, not a constant animation.\n\nYour previous RC5/code21 review opened successfully and category surfing was accepted. RC6 builds on that baseline. Typed signature verification is retained. A safe technical receipt is mirrored to the existing PR with readback after review; private feedback and all account/household/log details stay local. The local owner Review tab shows receipt sync and saves feedback in Obsidian without publishing it.\n"
package=out/'GHARTV_RC6_REVIEW.zip';prefix='GHARTV_RC6_REVIEW/'
