"""Actual Safari in an isolated cloud automation session. No owner accounts or provider media."""
...
record={'browser':'Safari','native_safari':True,'status':'STARTING','live_provider_playback':False,'owner_mac':False}
...
try:
 ...
 record['stage']='REAL_VIEWER_UI';action('POST','/url',{'url':BASE+'/'})
 ...
 record['stage']='DIRECT_BROWSER_FILMS';action('POST','/url',{'url':BASE+'/flixmomo.html'})
 wait_for(lambda:script("return document.getElementById('status') && !document.getElementById('status').textContent.includes('Checking')"))
 assert script("return document.getElementById('directProvider').href==='https://flixmomo.app/' && !document.getElementById('directProvider').hidden")
 script("document.getElementById('route').value='tor';document.getElementById('route').dispatchEvent(new Event('change'));return true")
 assert script("return document.getElementById('directProvider').hidden")
 ...
 shot('safari-films.png');record['optional_browser_and_explicit_route_ui']='PASS'
