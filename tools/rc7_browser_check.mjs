...
const home=await mkdtemp(join(tmpdir(),'ghartv-rc7-browser-')),origin='http://127.0.0.1:8874',sha='a'.repeat(40);
const receipt={run_id:'GHARTV-CYAN-8-20260915T140000Z-100',review_source:sha,version:'0.6.0-rc7-network-diagnostics',version_code:23,unsigned_apk_sha256:'b'.repeat(64),signed_apk_sha256:'c'.repeat(64),production_source:'d'.repeat(40),status:'REVIEW_READY',emulator:'RC7_INSTALLED_BYTES_VERIFIED_AND_FOREGROUND',obsidian:'WRITTEN_AND_READBACK_VERIFIED'};
...
try{
 ...
 await mkdir('/tmp/ghartv-check-evidence',{recursive:true});await page.screenshot({path:'/tmp/ghartv-check-evidence/rc7-browser-guide-fixture.png',fullPage:true});
 ...
 await page.screenshot({path:'/tmp/ghartv-check-evidence/rc7-browser-player-fixture.png',fullPage:true});
 ...
 await page.setViewportSize({width:390,height:844});assert.ok(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1));await page.screenshot({path:'/tmp/ghartv-check-evidence/rc7-mobile-fixture.png',fullPage:true});
 ...
 const desk={ok:true,green:{state:'INSTALLED_REVIEW_READY',run_id:receipt.run_id,source:sha,version:receipt.version,version_code:23,sha256:receipt.signed_apk_sha256},blue:{state:'LAST_VERIFIED_PUBLIC',versionCode:17,versionName:'0.5.4-rc8',sourceCommit:'d'.repeat(40),verified_at:new Date().toISOString()},promotion:{state:'NOT_REQUESTED'}};
 await page.route('**/owner-api/release/**',r=>{if(r.request().method()==='POST'&&new URL(r.request().url()).pathname.endsWith('/publish')){publicationRequests++;const b=JSON.parse(r.request().postData());assert.equal(b.sha256,receipt.signed_apk_sha256);assert.equal(b.confirmation,'PUBLISH EXACT CODE 23');return r.fulfill({json:{...desk,publication:{state:'PUBLIC_UPDATE_VERIFIED'}}});}return r.fulfill({json:desk});});
 await page.goto(origin+'/owner.html');await page.waitForFunction(()=>document.querySelector('#reviewVersion').textContent.includes('code 23'));
 ...
 await page.screenshot({path:'/tmp/ghartv-check-evidence/rc7-owner-overview-fixture.png',fullPage:true});
 ...
 await page.getByRole('button',{name:'Televisions & messages',exact:true}).click();await page.screenshot({path:'/tmp/ghartv-check-evidence/rc7-owner-tvs-fixture.png',fullPage:true});
 ...
 await page.screenshot({path:'/tmp/ghartv-check-evidence/rc7-release-desk-fixture.png',fullPage:true});
 ...
 await page.screenshot({path:'/tmp/ghartv-check-evidence/rc7-owner-diagnostics-fixture.png',fullPage:true});
 ...
 console.log('RC7_BROWSER_MANUAL_REPORTS_NETWORK_LABELS_PUNJABI_SCOPE_RELEASE_APPROVAL=PASS');
