...
assert.ok(html.includes('Manual refresh only. Detailed event rows load only when you request them.'));
assert.ok(html.includes('This page can run on your Mac, but report data is read from the existing Cloudflare collector and D1 database.'));
assert.ok(!html.includes('setInterval(refresh'));
assert.ok(!html.includes('setInterval(messages'));
assert.ok(!html.includes('setInterval(localConfigStatus'));
assert.ok(!html.includes("document.addEventListener('visibilitychange'"));
const summaryRefresh=html.match(/async function refresh\(\)\{([\s\S]*?)\nasync function loadDetails/);assert.ok(summaryRefresh);assert.ok(!summaryRefresh[1].includes('/v1/admin/export'));
const detailRefresh=html.match(/async function loadDetails\(\)\{([\s\S]*?)\nfunction feedback/);assert.ok(detailRefresh);assert.ok(detailRefresh[1].includes('/v1/admin/export?days='));
