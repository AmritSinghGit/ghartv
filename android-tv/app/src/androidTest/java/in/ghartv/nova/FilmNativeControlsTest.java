...
/** Real Android WebView and production coordinator; local synthetic artwork only. */
...
public class FilmNativeControlsTest {
    ...
    @Test public void posterArtworkRemainsAndScanDoesNotResetFocus() throws Exception {
        document("https://flixmomo.app/search?q=test",posters());evaluate("(()=>{window.savedImage=document.images[0];return JSON.stringify({ok:true})})()");
        assertEquals("READY",evaluate(FilmPosterNavigation.script("scan")).getString("state"));assertEquals(0,evaluate(FilmPosterNavigation.script("focus")).getInt("index"));assertEquals(1,evaluate(FilmPosterNavigation.script("right")).getInt("index"));
        evaluate(FilmPosterNavigation.script("scan"));evaluate(FilmPosterNavigation.script("scan"));JSONObject s=evaluate("JSON.stringify({same:savedImage===document.images[0],focus:document.activeElement.id,images:document.images.length,styles:document.querySelectorAll('#ghartv-poster-focus-v1').length})");
        assertTrue(s.getBoolean("same"));assertEquals("two",s.getString("focus"));assertEquals(2,s.getInt("images"));assertEquals(1,s.getInt("styles"));
    }
    @Test public void slugLabelIsReadableWithoutReplacingPoster() throws Exception {document("https://flixmomo.app/search?q=test","<a id='one' href='/movie/title'><img width='100' height='160' alt='harry-potter-and-the-goblet-of-fire'></a>");evaluate(FilmPosterNavigation.script("scan"));assertEquals("Harry Potter And The Goblet Of Fire",evaluate(FilmPosterNavigation.script("focus")).getString("label"));JSONObject s=evaluate("JSON.stringify({images:document.images.length,buttons:document.querySelectorAll('button').length,alt:document.images[0].alt})");assertEquals(1,s.getInt("images"));assertEquals(0,s.getInt("buttons"));assertEquals("harry-potter-and-the-goblet-of-fire",s.getString("alt"));}
    @Test public void firstRowUpReturnsToNativeSearch() throws Exception {document("https://flixmomo.app/search?q=test","<a href='/movie/title'><img width='100' height='160' alt='Display Title'></a>");evaluate(FilmPosterNavigation.script("scan"));evaluate(FilmPosterNavigation.script("focus"));assertEquals("TOOLBAR",evaluate(FilmPosterNavigation.script("up")).getString("state"));}
    ...
    private void attachControls(){ui(()->{
        ...
        controls.attach(browser);browser.requestFocus();controls.finished();
    });SystemClock.sleep(450);}
    @Test public void fullCoordinatorNeverReplacesPostersWithNativeTiles() throws Exception {
        document("https://flixmomo.app/search?q=test",posters());evaluate("(()=>{window.savedImages=Array.from(document.images);return JSON.stringify({ok:true})})()");attachControls();
        ui(()->controls.finished());SystemClock.sleep(2200);ui(()->controls.finished());SystemClock.sleep(250);
        JSONObject result=evaluate("JSON.stringify({same:savedImages.every((x,i)=>x===document.images[i]),metadata:document.querySelectorAll('small').length,buttons:document.querySelectorAll('button').length})");
        assertTrue(result.getBoolean("same"));assertEquals(2,result.getInt("metadata"));assertEquals(0,result.getInt("buttons"));ui(()->assertFalse(controls.visible()));
        android.graphics.Bitmap shot=InstrumentationRegistry.getInstrumentation().getUiAutomation().takeScreenshot();
        if(shot!=null){try(java.io.FileOutputStream stream=new java.io.FileOutputStream(new java.io.File(activity.getExternalFilesDir(null),"poster-review-fixture.png"))){shot.compress(android.graphics.Bitmap.CompressFormat.PNG,100,stream);}shot.recycle();}
    }
    @Test public void actualCoordinatorRemoteActivatesSelectedOriginalPoster() throws Exception {
        document("https://flixmomo.app/search?q=test",posters());attachControls();evaluate(FilmPosterNavigation.script("focus"));
        ui(()->assertTrue(controls.posterKey(new KeyEvent(KeyEvent.ACTION_DOWN,KeyEvent.KEYCODE_DPAD_RIGHT))));SystemClock.sleep(250);
        ui(()->assertTrue(controls.posterKey(new KeyEvent(KeyEvent.ACTION_DOWN,KeyEvent.KEYCODE_DPAD_CENTER))));SystemClock.sleep(300);assertEquals("https://flixmomo.app/movie/two",navigated);
    }
    @Test public void advertisedProviderOriginAndTabControlsAreRecognized() throws Exception {
        String url="https://flixmomo.bet/movie/one";document(url,"<a href='#one' onclick=\"document.title='player1'\">Player 1</a><div role='tab' tabindex='0'>Player 2</div><a href='https://unrelated.invalid/'>Player 6</a>");
        assertTrue(FlixMomoActivity.allowedTop(android.net.Uri.parse(url)));JSONObject p=evaluate(FilmPageSnapshot.read());assertEquals(2,p.getJSONArray("players").length());assertEquals("SELECTION_REQUESTED",evaluate(FilmPageSnapshot.select(url,0,"Player 1")).getString("state"));
    }
}
