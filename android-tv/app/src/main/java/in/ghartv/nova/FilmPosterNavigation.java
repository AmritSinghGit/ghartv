...
/** Remote focus on existing provider poster links. Does not replace artwork,
 * rebuild the result screen, inspect player iframes or change access controls. */
final class FilmPosterNavigation {
    private static final String JS = """
        (()=>{
          ...
          const label=a=>{
            const h=a.querySelector('h1,h2,h3,h4,[class*="title"]');
          ...
          if(!document.getElementById('ghartv-poster-focus-v1')){
            ...
            style.textContent='[data-ghartv-poster-focus="true"]{outline:4px solid #8cf4d5!important;outline-offset:4px!important;box-shadow:0 0 0 7px rgba(5,18,26,.85)!important;scroll-margin:30px 16px!important;}';
            (document.head||document.documentElement).appendChild(style);
          }
          const list=candidates();
          if(!list.length)return pack({state:'UNSUPPORTED',count:0});
          ...
          if(edit)return pack({state:'EDITING'});
          ...
          function focus(i){
            ...
            const p=list[i];p.node.setAttribute('data-ghartv-poster-focus','true');
            p.node.focus({preventScroll:true});p.node.scrollIntoView({block:'nearest',inline:'nearest',behavior:'auto'});
            return pack({state:'FOCUSED',index:i,count:list.length,label:p.label});
          }
          if(index<0)return focus(0);
          if(action==='focus')return focus(index);
          if(action==='activate'){
          ...
          const r=list[index].node.getBoundingClientRect(),cx=r.left+r.width/2,cy=r.top+r.height/2;
          let best=-1,score=Infinity;
          for(let i=0;i<list.length;i++){
            if(i===index)continue;
            const b=list[i].node.getBoundingClientRect(),dx=b.left+b.width/2-cx,dy=b.top+b.height/2-cy;
            const horizontal=action==='left'||action==='right';
            const forward=action==='left'?-dx:action==='right'?dx:action==='up'?-dy:dy;
            ...
            const candidate=forward+cross*3+(sameAxis?0:100000);
    ...
    static String script(String action) {
        switch(action){case "scan":case "focus":case "left":case "right":case "up":case "down":case "activate":break;
            default:throw new IllegalArgumentException("Unknown poster action");}
