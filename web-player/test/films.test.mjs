...
test('Review command checks native component hash and has a TV-only mode',async()=>{const s=await source('../../tools/owner_review.command.in');assert.match(s,/NATIVE_FILM_COMPONENT_CHECKSUM_FAILED/);assert.match(s,/--tv-only/);assert.match(s,/--films-only/);assert.match(s,/mac_window_observed/);assert.match(s,/version_code=29/);});
