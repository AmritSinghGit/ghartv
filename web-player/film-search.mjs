...
export async function findBrowser(){
 const candidates=[process.env.GHARTV_PROVIDER_BROWSER_EXECUTABLE,'/Applications/Brave Browser.app/Contents/MacOS/Brave Browser','/Applications/Chromium.app/Contents/MacOS/Chromium','/Applications/Google Chrome.app/Contents/MacOS/Google Chrome','/usr/bin/brave-browser','/usr/bin/chromium','/usr/bin/chromium-browser'];
...
export function makeFilmRoute({discover=findBrowser,start=spawn}={}){
 ...
 async function execute(input){
  ...
  const executable=await discover();if(!executable)throw Error('INSTALL_BRAVE_OR_CHROMIUM_FIRST');
  const proxy=input.route==='tor'?await detectTor():'socks5://127.0.0.1:9050';
  const profile=await mkdtemp(join(tmpdir(),'ghartv-film-'));busy=true;
  ...
  return new Promise(resolve=>{
   ...
   const finish=async value=>{if(done)return;done=true;clearTimeout(timer);if(input.action!=='open'||!value.ok){await stop();}busy=false;resolve(value);};
 ...
 return async function filmRoute(req,res,url,authorized,nonce){
  if(url.pathname!=='/flixmomo.html'&&!url.pathname.startsWith('/owner-api/films/'))return false;
  ...
  if(url.pathname==='/owner-api/films/status'&&req.method==='GET'){
  ...
  try{
   ...
   if(!['search','open'].includes(action)||!['direct','tor'].includes(body.route))throw Error('ACTION_OR_ROUTE_REJECTED');
   let input;
...
$('route').onchange=()=>{$('frame').removeAttribute('src');$('player').style.display='none';render();message($('route').value==='tor'?'Tor selected. Embedded direct playback is disabled. A verified Tor browser session is required.':'Direct HTTPS selected explicitly.');};
